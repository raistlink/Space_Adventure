...
(define (collision bulletlist enemy world)
  (let loop ((rest bulletlist))
    (if (null? rest)
        #f
        (if (or (> (shot-posx (car rest)) (enemy-posx enemy)) (> (+ (shot-posx (car rest)) 5) (enemy-posx enemy)))
               ...
               (if (> (shot-posy (car rest)) (-(enemy-posy enemy) 20))
                   (if (< (shot-posy (car rest)) (enemy-posy enemy))
                       (begin (shot-posy-set! (car rest) -10.0)
                              (begin (world-score-set! world (+ (world-score world) 10))
                                     #t))
                       (loop (cdr rest)))
                   (loop (cdr rest)))
               (loop (cdr rest)))
...
(define (player-collision enemylist world)
  (let loop ((rest enemylist))
    (if (null? rest)
        #f
        (if (or (> (enemy-posx (car rest)) (world-position world)) (> (+ (enemy-posx (car rest)) 20) (world-position world)))
